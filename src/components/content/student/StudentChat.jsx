                    ...
                    >
                        <Card.Body className="py-3">
                            <div className="d-flex justify-content-between align-items-start gap-2">
                                <Card.Title className="h6 mb-1">{inst.name}</Card.Title>
                                <Badge bg={badgeVariant} className="text-white">
                                    {badgeVariant === 'info' ? 'Lessons' : 'Inbox'}
                                </Badge>
                            </div>
                            <Card.Text className="small text-muted mb-0">{inst.preview}</Card.Text>
                        </Card.Body>
                    </Card>
                );
            })}
    ...
    return (
        <Container fluid className="px-0" style={{ minHeight: 'calc(100vh - 4rem)' }}>
            <Row className="g-0" style={{ minHeight: 'calc(100vh - 4rem)' }}>
                <Col md={4} lg={3} className="border-end overflow-auto py-3 px-3 aria-student-panel">
                    <div className="d-flex align-items-center justify-content-between mb-2">
                        <h4 className="mb-0 text-primary">Messages</h4>
                        <Button as={Link} to="/student/discover" variant="outline-primary" size="sm">
                            Find instructors
                        </Button>
                    </div>
                    <p className="small text-muted mb-0">Chat with your Aria instructors in one place.</p>
                ...
                <Col md={8} lg={9} className="d-flex flex-column py-3 px-3 aria-chat-main">
                    <div className="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                        <div>
                            <h4 className="mb-0">{selectedInstructor.name}</h4>
                            <span className="text-muted small">Private music lessons on Aria</span>
                        </div>
                        <Button as={Link} to={`/student/instructor/${selectedInstructor.id}`} variant="light" className="border">
                            View profile
                        </Button>
                    </div>
